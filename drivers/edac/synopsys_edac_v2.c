 ...
 * This driver is based on synopsys_edac.c driver
 *
...
#define ECC_ENABLED			0x4
#define ECC_ENABLED_X4_X8		0x5
...
#define	DDR_CE_INTR_MASK		0x1
#define	DDR_UE_INTR_MASK		0x2
#define	ECC_CE_UE_INTR_MASK		0x3
...
/**
 * synopsis_get_error_info - Get the current ECC error info.
 * @priv:	DDR memory controller private instance data.
 *
 * Return: one if there is no error otherwise returns zero.
 */
static int synopsis_get_error_info(struct synps_edac_priv *priv)
{
...
/**
 * synopsis_get_dtype - Return the controller memory width.
 ...
 */
static enum dev_type synopsis_get_dtype(const void __iomem *base)
{
	enum dev_type dt;
	u32 width;

	width = readl(base + MSTR_OFST);
...
/**
 * synopsis_get_ecc_state - Return the controller ECC enable/disable status.
 * @base:	DDR memory controller base address.
 *
 ...
 */
static bool synopsis_get_ecc_state(void __iomem *base)
{
	enum dev_type dt;
	u32 ecctype;

	dt = synopsis_get_dtype(base);
	if (dt == DEV_UNKNOWN)
		return false;

	ecctype = readl(base + ECC_CFG0_OFST) & SCRUB_MODE_MASK;
	if (ecctype == ECC_ENABLED)
		return true;
...
/**
 * synopsis_get_mtype - Returns controller memory type.
 ...
 */
static enum mem_type synopsis_get_mtype(const void __iomem *base)
{
	enum mem_type mt;
	u32 memtype;

	memtype = readl(base + MSTR_OFST);
...
static const struct synps_platform_data synopsis_edac_def = {
	.get_error_info	= synopsis_get_error_info,
	.get_mtype	= synopsis_get_mtype,
	.get_dtype	= synopsis_get_dtype,
	.get_ecc_state	= synopsis_get_ecc_state,
};
...
static void setup_column_address_map(struct synps_edac_priv *priv, u32 *addrmap)
{
	...
	memtype = readl(priv->baseaddr + MSTR_OFST);
